#!/usr/bin/env bash
#
# Threatos Linux ARM build-script for NanoPi NEO Plus2 (64-bit)
...
# More information: https://threatcode.github.io/docs/arm/nanopi-neo-plus2/
#
...
eatmydata apt-get install -y threatos-sbc-allwinner linux-image-arm64 firmware-brcm80211
...
sed -i -e "s|.*GNU/Linux Rolling|menu label Threatos Linux|g" ${work_dir}/boot/extlinux/extlinux.conf

status "Set the default options in /etc/default/u-boot"
echo 'U_BOOT_MENU_LABEL="Threatos Linux"' >>${work_dir}/etc/default/u-boot
