#!/usr/bin/env bash
#
# Threatos Linux ARM build-script for Utilite Pro (32-bit)
# Source: https://github.com/threatcode/threatos
#
# This is a community script - you will need to generate your own image to use
# More information: https://threatcode.github.io/docs/arm/utilite-pro/
#
...
image_name=${3:-threatos-linux-$1-utilite-pro}
...
if [[ $EUID -ne 0 ]]; then
    ...
    echo "Use: sudo $0 ${1:-2.0} ${2:-threatos}" >&2
...
# Pass version number
if [[ $# -eq 0 ]]; then
    echo "Please pass version number, e.g. $0 2.0, and (if you want) a hostname, default is threatos" >&2
...
if [ ! -e "bsp" ]; then
    ...
    echo "Please clone the full repository ${threatosgit}/build-scripts/threatos-arm" >&2
...
# Working directory
work_dir="${base_dir}/threatos-${architecture}"
...
--keyring=/usr/share/keyrings/threatos-archive-keyring.gpg \
--include=threatos-archive-keyring,eatmydata \
--components=${components} \
--arch ${architecture} ${suite} ${work_dir} hhttps://threatcode.github.io/threatos
...
# Mirror replacement
if [[ ! -z "${@:5}" || "$suite" != "threatos-rolling" ]]; then
    mirror=${@:5}
    [ ! -z "${@:5}" ] || mirror="hhttps://threatcode.github.io/threatos"
    [ "$suite" != "threatos-rolling" ] && suite=threatos-rolling
...
# Clone a cross compiler to use instead of the Threatos one due to kernel age
