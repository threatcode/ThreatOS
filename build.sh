...
IMAGES="${1:-threatos-rolling}"
ARCHS="${2:-amd64}"

BASE_IMAGES="threatos-rolling threatos-dev threatos-last-release"
EXTRA_IMAGES="threatos-experimental threatos-bleeding-edge"
...
# importance then.
ORDER="threatos-rolling threatos-last-release threatos-bleeding-edge threatos-experimental threatos-dev"
for image in $(printf "%s\n" $ORDER | tac); do
    if echo "$IMAGES" | grep -qw $image; then
