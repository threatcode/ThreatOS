#!/usr/bin/env bash
#
# Threatos Linux ARM build-script (host machine preparation)
...
if [[ $EUID -ne 0 ]]; then
    ...
    echo -e "\n[i] Threatos-ARM build-script machine preparation"
...
if [ ! -f /usr/share/keyrings/threatos-archive-keyring.gpg ]; then
    echo -e "\n[i] Installing: threatos-archive-keyring"
    temp_key="$(mktemp -d)"
    git clone https://gitlab.com/kalilinux/packages/kali-archive-keyring.git $temp_key
    cd $temp_key/
    make
    cp -v threatos-archive-keyring.gpg /usr/share/keyrings/ #make install
    cd $OLDPWD/
    rm -rf $temp_key

else
    echo -e "\n[i] Already have: threatos-archive-keyring"
