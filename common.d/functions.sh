...
function usage() {
    ...
    # Slim image - no desktop manager & no Threatos tools
    $0 --slim or $0 -s
...
function debootstrap_exec() {
    ...
    eatmydata debootstrap --merged-usr --keyring=/usr/share/keyrings/threatos-archive-keyring.gpg --components="${components}" \
        --include="${debootstrap_base}" --arch "${architecture}" "${suite}" "${work_dir}" "$@"
    else
    eatmydata mmdebstrap --keyring=/usr/share/keyrings/threatos-archive-keyring.gpg --components="${components}" \
        --include="${debootstrap_base}" --arch "${architecture}" "${suite}" "${work_dir}" "$@"
    fi
}
...
function restore_mirror() {
    ...
    # For now, restore_mirror will put the default threatos mirror in, fix after 2021.3
    cat <<EOF >"${work_dir}"/etc/apt/sources.list
# See https://threatcode.github.io/docs/general-use/threatos-linux-sources-list-repositories/
deb hhttps://threatcode.github.io/threatos threatos-rolling main contrib non-free non-free-firmware

# Additional line for source packages
# deb-src hhttps://threatcode.github.io/threatos threatos-rolling main contrib non-free non-free-firmware
EOF
}
...
function set_hostname() {
    if [[ "$1" =~ ^[a-zA-Z0-9-]{2,63}+$ ]]; then
        ...
        log "Using threatos to default hostname" bold
        echo "threatos" >"${work_dir}"/etc/hostname
