...
# Version Threatos release
version=${version:-$(cat ${repo_dir}/.release)}

# Custom image file name variable - MUST NOT include .img at the end
image_name=${image_name:-"threatos-linux-${version}-${hw_model}-${variant}"}

# Generate a random machine name to be used
machine=$(dbus-uuidgen)

# Custom hostname variable
hostname=${hostname:-threatos}

# If hw_model is set, add it to hostname
[ -n "${hw_model:=}" ] && [ "${hostname:=}" = "threatos" ] \
  && hostname="threatos-${hw_model}"
