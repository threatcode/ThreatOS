...
# Check for the threatos archive keyring
if [ ! -e /usr/share/keyrings/threatos-archive-keyring.gpg ]; then
    log "Error: missing threatos-archive-keyring" red
    log "Please download the latest version from" red
    log "https://threatos.download/threatos/pool/main/k/threatos-archive-keyring" red
...
if [ ! -e "bsp" ]; then
    ...
    log "Please clone the full repository ${threatosgit}/build-scripts/threatos-arm" green
