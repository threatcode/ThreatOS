...
image=$1
architecture=$2
mirror=${3:-http://threatcode.github.io/threatos}
...
case $image in
    threatos-dev|threatos-rolling)
        distro=$image
        ;;
    threatos-last-release)
        distro=threatos-last-snapshot
        ;;
    *)
...
if [ ! -e /usr/share/keyrings/threatos-archive-keyring.gpg ]; then
    echo "ERROR: you need /usr/share/keyrings/threatos-archive-keyring.gpg" >&2
    echo "ERROR: install threatos-archive-keyring" >&2
    exit 1
fi
...
debootstrap --variant=minbase --components=main,contrib,non-free,non-free-firmware \
    --arch="$architecture" --include=threatos-archive-keyring \
...
rootfs_chroot apt-get -y --no-install-recommends install threatos-defaults
...
if [ "$image" = "threatos-last-release" ]; then
