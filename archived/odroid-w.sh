#!/bin/bash
#
# Threatos Linux ARM build-script for ODROID-W
...
imagename=${3:-threatos-linux-$1-odroidw}
...
desktop="threatos-desktop-xfce threatos-root-login xfonts-terminus xinput \
...
wget -q ${githubraw}/Re4son/RPi-Tweaks/master/threatospi-tft-config/threatospi-tft-config -O /usr/bin/threatospi-tft-config
chmod 755 /usr/bin/threatospi-tft-config
...
eatmydata apt-get update
eatmydata apt-get install -y \$aptops threatospi-kernel threatospi-bootloader threatospi-re4son-firmware threatospi-kernel-headers
