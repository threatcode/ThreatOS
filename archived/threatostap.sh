...
imagename=${3:-threatos-linux-$1-threatostap}

# Suite to use, valid options are:
# threatos-rolling, threatos-dev, threatos-bleeding-edge, threatos-dev-only, threatos-experimental, threatos-last-snapshot
suite=${suite:-"threatos-rolling"}
...
mirror=${mirror:-"hhttps://threatcode.github.io/threatos"}

# Gitlab URL Threatos repository
threatosgit="https://github.com/threatcode"
...
if [[ $EUID -ne 0 ]]; then
    ...
    echo "Use: sudo $0 ${1:-2.0} ${2:-threatos}"
...
# Pass version number
if [[ $# -eq 0 ]]; then
    echo "Please pass version number, e.g. $0 2.0, and (if you want) a hostname, default is threatos"
...
if [ ! -e "bsp" ]; then
    ...
    echo "Please clone the full repository ${threatosgit}/build-scripts/threatos-arm"
...
# Base directory
basedir=${repo_dir}/threatostap-"$1"

# Working directory
work_dir="${basedir}/threatos-${architecture}"
...
arm="threatos-linux-arm ntpdate"
...
desktop="threatos-desktop-xfce xfonts-terminus xinput xserver-xorg-video-fbdev"

tools="threatos-linux-default"

services="apache2 atftpd"

extras="alsa-utils bc bison bluez bluez-firmware threatos-linux-core \
...
eatmydata debootstrap --foreign --keyring=/usr/share/keyrings/threatos-archive-keyring.gpg --include=threatos-archive-keyring,eatmydata \
    --components=${components} --arch ${architecture} ${suite} ${work_dir} hhttps://threatcode.github.io/threatos
...
# Create threatos user with threatos password... but first, we need to manually make some groups because they don't yet exist...
...
groupadd -g 1000 threatos

useradd -m -u 1000 -g 1000 -G sudo,audio,bluetooth,cdrom,dialout,dip,lpadmin,netdev,plugdev,scanner,video,threatos -s /bin/bash threatos
...
git clone --depth 1 https://github.com/wawtechnologies/linux-kernel-3.14.51-catchwire-threatostap.git ${work_dir}/usr/src/kernel
cd ${work_dir}/usr/src/kernel
git rev-parse HEAD >${work_dir}/usr/src/kernel-at-commit
patch -p1 --no-backup-if-mismatch <${repo_dir}/patches/threatos-wifi-injection-3.14.patch
...
cp ${repo_dir}/kernel-configs/threatostap.config .config
cp ${repo_dir}/kernel-configs/threatostap.config ${work_dir}/usr/src/threatostap.config
...
git clone https://github.com/wawtechnologies/utilities-catchwire-threatostap "${basedir}"/utilities
...
#u-boot threatostap specific overrides:
...
git clone https://github.com/wawtechnologies/u-boot-2014.04-catchwire-threatostap.git "${basedir}"/u-boot
cd "${basedir}"/u-boot
